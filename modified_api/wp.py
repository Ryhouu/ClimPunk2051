...
class WP(Draggable):
    def __init__(self, title='', content=[], scale_x=1,**kwargs):
        super().__init__(
            origin=(-0,.5), 
            scale=(scale_x, Text.size*2), 
            color=color.black,
            position=(0, 0.2))
    ...
    def layout(self):
        ...
        self.panel.model = Quad(
            # aspect=self.panel.world_scale_x/(self.panel.world_scale_y), 
            radius=.05)
